...
def abs_sobel_adapthresh(img, orient, sobel_kernel, adap_size, adap_c):
    ...
    abs_sobel = np.absolute(sobel)
    scaled_sobel = np.uint8(255 * abs_sobel / np.max(abs_sobel))
    binary = cv2.adaptiveThreshold(scaled_sobel, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C,
...
def hls_s_adapthresh(img, adap_size, adap_c):
    hls = cv2.cvtColor(img, cv2.COLOR_BGR2HLS)
    _, _, s = cv2.split(hls)
    binary = cv2.adaptiveThreshold(s, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, adap_size, adap_c)
    return binary
